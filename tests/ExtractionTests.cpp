...
static void testNoOverlapDoesNotCreateOutput()
{
    ...
    const GPST start{2300, 1.0};
    const GPST end{2300, 2.0};
    const int result = fastExtractByGPST(input, output, start, end);
    expect(result == -3, "no-overlap extraction returns -3");
    expect(!fileExists(output), "no-overlap extraction creates no output");
